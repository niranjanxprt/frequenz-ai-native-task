...
def run_streamlit_app(port: int, run_dir: Path, streamlit_dir: Path):
    ...
    # Correct path to the basic app within src/
    app_file = "src/apps/app.py"
    pid_file = run_dir / f"streamlit_basic.{port}.pid"
    log_file = run_dir / f"streamlit_basic.{port}.log"
    ...
    print(f"🚀 Starting basic Streamlit app on port {port}...")
    ...
    try:
        ...
        with open(log_file, "w") as log:
            process = subprocess.Popen(
                cmd, stdout=log, stderr=subprocess.STDOUT, env=env, cwd=Path.cwd()
            )
        ...
        print("✅ Basic app started successfully!")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run basic Streamlit app with branding"
    )
    parser.add_argument(
        "--port", "-p", type=int, default=8501, help="Port to run on (default: 8501)"
    )
    parser.add_argument(
    ...
    # Kill mode
    if args.kill:
        pid_file = run_dir / f"streamlit_basic.{args.port}.pid"
        kill_existing_process(pid_file)
