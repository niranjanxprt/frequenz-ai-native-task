...
class RepositoryContext:
    ...
    @classmethod
    def from_gitingest_file(cls, file_path: str = "data/frequenz-floss-frequenz-sdk-python-LLM.txt") -> Optional['RepositoryContext']:
        ...
        try:
            ...
            dependencies = []
            pyproject_match = re.search(r'pyproject\.toml.*?\n(.*?)(?=File:|=====|\Z)', content, re.DOTALL)
            if pyproject_match:
    ...
    def from_gitingest(cls, repo_url: str) -> 'RepositoryContext':
        ...
        try:
            ...
            for line in tree.split('\n'):
                if '.' in line and line.strip():
                    ...
                    if '.' in filename:
                        ...
                        if ext in ['py', 'js', 'ts', 'java', 'cpp', 'c', 'go', 'rs', 'rb', 'toml', 'yml', 'yaml', 'md']:
                            languages.add(ext)
            ...
            if pyproject_match:
                ...
                deps_section = re.search(r'\[tool\.poetry\.dependencies\](.*?)(?=\[|\Z)', toml_content, re.DOTALL)
                if deps_section:
            ...
            structure = {
                'has_tests': any(word in content.lower() for word in ['test_', 'tests/', 'pytest', 'unittest']),
                'has_docs': any(word in content.lower() for word in ['docs/', 'readme', 'documentation', 'sphinx']),
                'has_ci': any(word in content.lower() for word in ['.github', 'ci.yml', 'workflow', 'action']),
                'has_docker': 'dockerfile' in content.lower() or 'docker-compose' in content.lower(),
            ...
            for line in tree.split('\n'):
                ...
                if any(main in line_lower for main in ['main.py', 'app.py', '__init__.py', '__main__.py']):
                    structure['main_files'].append(line.strip())
                if any(conf in line_lower for conf in ['config', 'settings', '.env', 'pyproject.toml', '.yml', '.yaml']):
                    structure['config_files'].append(line.strip())
            
            # Detect if it's a Python package
            structure['is_python_package'] = 'setup.py' in content.lower() or 'pyproject.toml' in content.lower()
...
class AdvancedQueryEngine:
    ...
    def __init__(self, knowledge_path: str = "project_knowledge.jsonld", repo_context: Optional[RepositoryContext] = None):
    ...
    def enhance_buckets(self) -> Dict[str, List[str]]:
        ...
        if self.repo_context:
            ...
            for lang in self.repo_context.languages:
                if lang == 'py':
                    enhanced["example"].extend(["python", "def", "class", "import", "async"])
                elif lang == 'toml':
            ...
            enhanced["dependencies"].extend([dep.lower() for dep in self.repo_context.dependencies])
            ...
            if self.repo_context.structure.get('frequenz_patterns'):
                ...
                if patterns.get('has_actors'):
                    enhanced["features"] = enhanced.get("features", []) + ["actor", "orchestration"]
                if patterns.get('has_channels'):
                    enhanced["features"] = enhanced.get("features", []) + ["channel", "communication"]
                if patterns.get('has_microgrid'):
                    enhanced["features"] = enhanced.get("features", []) + ["microgrid", "energy"]
                if patterns.get('has_timeseries'):
                    enhanced["features"] = enhanced.get("features", []) + ["timeseries", "data", "streaming"]
                if patterns.get('has_battery') or patterns.get('has_pv') or patterns.get('has_ev'):
                    enhanced["features"] = enhanced.get("features", []) + ["battery", "pv", "ev", "charger"]
    ...
    def analyze_repository_structure(self) -> str:
        ...
        if not self.repo_context:
            return "No repository context available. Use --repo to analyze a repository."
        
        ctx = self.repo_context
        analysis = [
            f"🔍 Frequenz SDK Python Repository Analysis",
    ...
    def enhanced_semantic_search(self, question: str, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    def _search_repository_content(self, question: str, top_k: int = 3) -> List[Dict[str, Any]]:
    ...
    def answer_question(self, question: str, use_enhanced: bool = True) -> Tuple[str, str, List[Dict[str, Any]]]:
        """Answer question with optional enhanced context."""
        # Special handling for structure analysis
        if any(word in question.lower() for word in ['structure', 'layout', 'organization', 'architecture']):
        ...
        if use_enhanced and (self.repo_context or self.retriever):
            ...
            if search_results:
                ...
                if best_result['source'] == 'json-ld':
                    ...
                    answer_parts.append(f"📁 From Repository Code:\n{best_result['text']}")
    ...
    def _get_relevant_repo_context(self, question: str) -> str:
        ...
        if any(word in q_lower for word in ['install', 'setup', 'dependencies', 'require']):
            if ctx.dependencies:
...
def main():
    parser = argparse.ArgumentParser(description='Advanced Frequenz SDK query system with repository analysis')
    parser.add_argument('question', nargs='*', help='Natural language question')
    parser.add_argument('--repo', default='https://github.com/frequenz-floss/frequenz-sdk-python', 
                       help='Repository URL (default: Frequenz SDK Python)')
    parser.add_argument('--knowledge', default='project_knowledge.jsonld', help='Knowledge file path')
    parser.add_argument('--analyze-structure', action='store_true', help='Analyze repository structure')
    parser.add_argument('--search-results', type=int, default=3, help='Number of search results to show')
    parser.add_argument('--basic', action='store_true', help='Use basic query without enhancements')
    parser.add_argument('--no-repo', action='store_true', help='Skip repository analysis')
    ...
    if not args.no_repo:
        if not GITINGEST_AVAILABLE:
            ...
            try:
                ...
                print(f"✅ Repository analyzed: {repo_context.file_count} files, {len(repo_context.languages)} languages\n")
            except Exception as e:
    ...
    try:
        answer_text, label, search_results = engine.answer_question(question, use_enhanced)
        ...
        if search_results and len(search_results) > 1:
            ...
            for i, result in enumerate(search_results[1:args.search_results], 1):
                ...
                print(f"{i}. {source_icon} [{result['label']}] (confidence: {confidence:.2f})")
...
def get_repository_summary() -> Optional[Dict[str, Any]]:
    ...
    if GITINGEST_AVAILABLE:
        try:
            ...
            summary, tree, content = ingest("https://github.com/frequenz-floss/frequenz-sdk-python")
...
def _extract_relevant_context(content: str, question: str, max_chars: int = 6000) -> str:
    ...
    search_patterns = {
        'install': ['installation', 'setup', 'pip install', 'requirements', 'pyproject.toml', 'getting started'],
    ...
    if best_pattern:
        ...
        for i, line in enumerate(lines):
            if any(keyword in line.lower() for keyword in search_patterns[best_pattern]):
