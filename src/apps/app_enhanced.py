    ...
    if st.session_state.gitingest_context is None:
        try:
            ...
            if hasattr(query_advanced, 'get_repository_summary'):
                st.session_state.gitingest_context = query_advanced.get_repository_summary()
            else:
...
def render_api_configuration():
    ...
    if env_key:
        ...
        st.sidebar.caption(f"Using key: {env_key[:8]}...{env_key[-4:] if len(env_key) >= 12 else env_key}")
    else:
    ...
    with st.sidebar.expander("🔧 Override API Key", expanded=not env_key):
        frontend_key = st.text_input(
            ...
            value="",
            type="password",
            help="Get your free API key from: https://www.perplexity.ai/settings/api\n(Leave empty to use .env file)" if env_key else "Get your free API key from: https://www.perplexity.ai/settings/api",
    ...
    if st.session_state.perplexity_api_key:
        key_source = "ENV file" if st.session_state.get('using_env_key', False) else "Frontend input"
        st.sidebar.success(f"✅ **Perplexity {st.session_state.selected_model}**: Connected")
...
def get_fallback_answer(question: str, data: Dict) -> Optional[str]:
    ...
    for qobj in (data.get("subjectOf") or []):
        if isinstance(qobj, dict) and qobj.get("@type") == "Question":
            ...
            # Simple matching
            if any(word in question.lower() for word in kg_question.split() if len(word) > 3):
                return answer
    return None
...
def process_question(question: str, perplexity_api, enhanced_context: str, data: Dict):
    ...
    if perplexity_api:
        ...
        if GITINGEST_AVAILABLE and hasattr(query_advanced, 'get_relevant_repository_context'):
            try:
                repo_context = query_advanced.get_relevant_repository_context(question)
                if repo_context:
                    question_specific_context = enhanced_context + f"\n\nRelevant Repository Context:\n{repo_context}"
            except Exception as e:
                # Fall back to original context if there's an error
                pass
        
        with st.spinner(f"🔍 Analyzing with Perplexity {st.session_state.selected_model} (Real-time Web Search)..."):
        ...
        with st.spinner("📚 Getting answer from knowledge graph..."):
            ...
            if answer:
                ...
                if "example" in question.lower():
                    ...
                    st.write(answer)
                st.caption("Source: Local Knowledge Graph • Add Perplexity Sonar API for AI responses!")
            else:
                st.warning("⚠️ **Add Perplexity Sonar API key** in sidebar for intelligent responses!")
...
def main():
    ...
    with col2:
        ...
        st.caption("Streamlined chat interface • Powered by Perplexity Sonar with Real-time Web Search & Citations")
    
    # Sidebar configuration
    with st.sidebar:
        # Logo
        logo_candidates = glob.glob("assets/*logo*.png") + glob.glob("assets/*frequenz*.png")
        if logo_candidates:
            st.image(logo_candidates[0], width=180)
        
        api_connected = render_api_configuration()
    ...
    st.markdown("Ask anything about the Frequenz SDK - powered by real-time web search & citations")
        ...
        with col1:
            if st.button("🚀 Get AI Answer", type="primary", use_container_width=True):
                if user_question.strip():
                    ...
                    process_question(user_question, perplexity_api, enhanced_context, data)
        
        with col2:
            if st.button("🔄 Clear History", disabled=not st.session_state.chat_history):
    ...
    if st.session_state.chat_history:
        ...
        with st.expander(f"📖 Recent Conversations ({chat_count}/10 saved)", expanded=False):
            ...
            for i, chat in enumerate(reversed(st.session_state.chat_history)):
                ...
                if i < chat_count - 1:
                    st.markdown("<hr style='border-color: #444; opacity: 0.5; margin: 25px 0;'>", unsafe_allow_html=True)
    ...
    if not perplexity_api:
        ...
        **🌟 Get Perplexity Sonar API:**
        ...
        st.success(f"✅ **Optimal Setup!** Using Perplexity {st.session_state.selected_model} with GitIngest context")
